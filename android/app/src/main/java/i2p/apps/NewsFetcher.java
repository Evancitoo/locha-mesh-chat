...
//                        NewsActivity.class);
    ...
    private File processSU3() throws IOException {
        ...
        try {
            ...
            net.i2p.router.news.NewsXMLParser parser = new net.i2p.router.news.NewsXMLParser(_context);
            parser.parse(xml);
            xml.delete();
            net.i2p.router.news.NewsMetadata data = parser.getMetadata();
     ...
     *  @since 0.9.20
     */
    private void outputOldNewsXML(net.i2p.router.news.NewsMetadata data, List<net.i2p.router.news.NewsEntry> entries,
                                  String sudVersion, String signingKeyName, File to) throws IOException {
        net.i2p.router.news.NewsMetadata.Release latestRelease = data.releases.get(0);
        Writer out = null;
        try {
            ...
            String su3Torrent = "";
            String su2Torrent = "";
            for (net.i2p.router.news.NewsMetadata.Update update : latestRelease.updates) {
            ...
            if (entries == null)
                return;
            for (net.i2p.router.news.NewsEntry e : entries) {
